...
### 数据库开发说明
- 当前使用 SQLite，默认数据库文件为 `ai_world_engine.db`
- `ai_world_engine.db` 不提交到 Git（已加入 .gitignore）
- 当前项目暂未引入 Alembic 数据库迁移工具
- 开发阶段如果模型字段变更导致旧本地数据库报错，可删除 `ai_world_engine.db` 后重启项目重新生成
- 后续正式版本可考虑加入数据库迁移工具
