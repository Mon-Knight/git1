...
## 2026-05-11 — v1.7.1: 防混乱开发规范与项目路线护栏补齐

### 为什么 v1.7.0 后不直接做 v1.8.0 分卷大纲

v1.7.0 完成了全书演化推演增强版，功能上已经可以基于主线方案生成分卷大纲。但项目存在以下问题：
1. 开发规范未建立，Agent 每次开发没有统一约束。
2. 应用级信息架构未明确，后续 UI 改造容易混乱。
3. 模块边界未定义，功能堆叠后职责混杂风险高。
4. 版本路线不清晰，容易做出超范围的"顺手功能"。

如果不先补齐这些基础设施，直接做分卷大纲会：
- 继续新增页面入口但缺乏统一导航
- Agent 可能在任务中顺手加未来功能
- 不同模块的职责边界越来越模糊
- 测试和文档维护成本持续升高
...
1. **Agent 需要明确的禁止清单**：不能删旧路由、不能顺手加功能、不能让 AI 自动写入正史。
2. **版本需要明确路线**：v1.7.x 是应用化补课阶段，v1.8.0 才是分卷大纲，避免跳版本。
3. **模块需要清晰边界**：世界资料库/小说工程/创作资产各有职责，不交叉污染。
4. **UI 需要信息架构**：先想清楚最后长什么样，再逐步改造，不做一次性重写。

### 为什么 UI 应用化要在正文生成前完成

1. 正文生成涉及大量新页面（分卷大纲、章节大纲、正文编辑、润色对比）。
2. 如果先堆正文功能再改造 UI，会面临大量模板重构。
3. 先有统一的应用壳（顶栏+侧栏），新功能页面直接放入合适位置。
