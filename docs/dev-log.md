...
v1.7.4 完成了世界控制台，但功能入口以"基础功能入口"形式堆叠展示，缺乏创作流程导向。随着功能增多（设定库、AI 推演、小说工程、创作资产、检查中心、数据设置），用户需要按创作流程找到对应功能。

### 分组包含哪些内容

7 个功能分组：
1. **设定库**：角色管理、势力管理、地点管理、规则管理
2. **剧情历史**：历史事件、时间线、正史时间线、非正史事件
3. **AI 推演**：开始 AI 推演、推演记录、分支记录
4. **小说工程**：全书演化推演、演化方案列表、分卷大纲（后续开放）、章节大纲（后续开放）、正文生成（后续开放）
5. **创作资产**：创作上下文总览、写作风格方案、剧情时间点、创作上下文包
6. **检查中心**：检查中心、设定矛盾检查、角色行为合理性检查、时间线冲突检查（后续开放）、正文一致性检查（后续开放）、风格一致性检查（后续开放）
7. **数据与设置**：导出当前世界、数据管理、AI 设置

### 哪些功能标注为后续版本开放

- **小说工程**：分卷大纲（v1.8.0）、章节大纲（v1.9.0）、正文生成（v2.0.0）
- **检查中心**：时间线冲突检查（后续）、正文一致性检查（后续）、风格一致性检查（后续）

### 二级导航

在世界标题区下方新增页面内锚点导航，包含：总览、设定库、剧情历史、AI 推演、小说工程、创作资产、检查中心、数据与设置。点击后页面内跳转。

### 旧路由保留确认

所有旧路由继续可访问，仅改变世界控制台的展示组织方式。

### 测试结果

- pytest tests/ -v：706 passed, 0 failed
- python -m compileall .：No errors
- python scripts/check_encoding.py：All passed
- python scripts/verify_desktop_build.py --all：ALL CHECKS PASSED
