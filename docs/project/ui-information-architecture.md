...
> **版本**: v2.0.1  
> **定位**: 小说工程核心化 — 从「世界项目为中心」调整为「小说工程为核心」
...
| **v2.0.0** | **正文草稿生成基础闭环** | ✅ |
| **v2.0.1** | **小说工程核心化 + 侧边栏折叠/可点击优化 + 详情页图标统一** | **✅ 当前** |
...
AI World Engine
├── 🏠 工作台
├── 📖 小说工程（核心入口，button切换，aria-expanded/controls）
├── 🌍 世界设定（原"世界项目"）
├── 📦 创作资产
├── 🤖 AI 推演
├── 🔍 质量检查
...
> v2.0.1 侧边栏优化：分组标题从 `<a>` 改为 `<button>`，移除 `disabled` 类，无世界时显示引导+CTA而非灰色禁用态。
