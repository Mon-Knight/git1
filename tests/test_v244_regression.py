"""
AI World Engine - v2.6.0 Regression Tests
...
class TestQualityReportsAvailable:
    """v2.6.0: Quality reports must not be locked."""
...
class TestStyleImportAvailable:
    """v2.6.0: TXT import entry must be visible."""
...
class TestOldStyleProfilesVisible:
    """v2.6.0: Old style profiles must be visible."""
...
class TestDiscardedSuggestionDelete:
    """v2.6.0: Discarded suggestions can be deleted."""
...
class TestVersionDisplay:
    """v2.6.0: Version must show 2.6.0."""

    def test_config_version_is_244(self):
        from app.config import settings
        assert settings.VERSION == "2.6.0"

    def test_home_shows_version(self, client):
        r = client.get("/")
        assert "2.6.0" in r.text
